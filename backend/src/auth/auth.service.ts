...
export class AuthService {
    ...
    async hostsignup({ Email, Password, Gender, phone, Emailauthentication }: CreateuserDto) {
        ...
        try {
            ...
            const user = this.userRepository.create({ registration_information: "site", email: Email, password: hashedPassword, nickname: Nickname, phone, gender: Gender, authority: Role.Host })
            return await this.userRepository.save(user)
        }
        catch (error) {
            if (error instanceof BadRequestException) {
