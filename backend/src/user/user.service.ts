    ...
    async upload(filename: string, file: Buffer, id: number) {
        ...
        try {
            await this.s3Client.send(
                new PutObjectCommand({ Bucket: 'chatpt-githubaction-s3-bucket', Key: fileName, Body: file })
            );
            await this.userRepository.update(id, {
                profile_image: 'https://chatpt-githubaction-s3-bucket.s3.ap-northeast-2.amazonaws.com/' + fileName,
            });
