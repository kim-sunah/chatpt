...
const KakaoRedirect = () => {
    ...
    useEffect(() => {
        ...
        .then(resData => 
            ...
            .then(res =>res.json()). 
            then(resData => fetch("http://localhost:4000/auth/kakaosingup", {method :"POST" , headers :{"Content-Type" : "application/json"}, body : JSON.stringify({Email:resData.kakao_account.email , Nickname : resData.kakao_account.profile.nickname , profile_image :resData.kakao_account.profile.profile_image_url})})
            .then(res => res.json())
            .then(resData => {
