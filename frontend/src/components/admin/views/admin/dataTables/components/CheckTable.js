...
export default function CheckTable(props) {
  ...
  useEffect(() => {
    fetch("https://iamchatpt.com:444/admin/reservationproductlist", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ pages: pages }) }).then(res => res.json()).then(resData => { setProductCount(resData.productCount); setproductlist(resData.products) }).catch(err => console.log(err))
    const socket = openSocket('https://iamchatpt.com:444', { transports: ['websocket'] });
    socket.on('events', (data) => {
      if (data === "acceptproduct" || data === "rejectproduct") {
        fetch("https://iamchatpt.com:444/admin/reservationproductlist", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ pages: pages }) }).then(res => res.json()).then(resData => { console.log(resData.products); setProductCount(resData.productCount); setproductlist(resData.products) }).catch(err => console.log(err))
      }
    });
  }, [pages])

  const productacceptance = (id) => {
    fetch(`https://iamchatpt.com:444/product/accept/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" } }).then(res => res.json()).then(resData => { }).catch(err => console.log(err))

  }
  const productreject = (id) => {
    fetch(`https://iamchatpt.com:444/product/${id}`, { method: "DELETE", headers: { "Content-Type": "application/json" } }).then(res => res.json()).then(resData => { }).catch(err => console.log(err))
