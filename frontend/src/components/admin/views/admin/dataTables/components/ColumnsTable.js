...
export default function ColumnsTable(props) {
  ...
  useEffect(() => {
    fetch("https://iamchatpt.com:444/admin/banuserList", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ pages: pages }) }).then(res => res.json()).then(resData => { setbanusercount(resData.userCount); setbanuserlist(resData.users) }).catch(err => console.log(err))
  ...
  const banuser = (id) => {
    fetch(`https://iamchatpt.com:444/admin/limituser/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" } }).then(res => res.json()).then(resData => console.log(resData)).catch(err => console.log(err))
