...
const Signup = () => {
  ...
  const Singupsubmithanlder = (events) => {
    ...
    if (hostsignup) {
      console.log("ads")
      fetch("http://localhost:4000/auth/hostsignup", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ Email: emailref.current.value, Password: passwordref.current.value, ConfirmPassword: Confirmpassword.current.value, phone: Phone.current.value, Gender: Gender, Emailauthentication: Emailauthentication.current.value }) })
        .then(res => res.json())
        .then(resData => {
          if (resData.statusCode !== 201) {
            seterrorCode(resData.message)
  ...
  return (
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-900">
      <div className="rounded-lg border bg-card text-card-foreground shadow-lg" data-v0-t="card">
        ...
        {errorCode && <div className="mt-4">
            <p class="text-red-500 text-center">{errorCode}</p>
          </div>}
        <div className="p-6">
          ...
          <form className="space-y-4" onSubmit={Singupsubmithanlder}>
            ...
            <div class="flex flex-col space-y-4">
              <button type="submit" onClick={() => { setnormalsignup(true); sethostsignup(false) }} className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover:bg-primary/90 h-10 px-4 py-2 bg-green-500 text-white">
                일반 회원가입
              </button>
              <button type="submit" onClick={() => { setnormalsignup(false); sethostsignup(true) }} className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover:bg-primary/90 h-10 px-4 py-2 bg-blue-500 text-white">
                판매자 회원가입
              </button>
            </div>
          </form>
