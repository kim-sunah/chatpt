...
const Signup = () => {
  ...
  const emailsubmit = (events) => {
    events.preventDefault()
    fetch("iamchatpt.com/auth/Email_authentication", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ Email: emailref.current.value }) })
      .then(res => res.json())
