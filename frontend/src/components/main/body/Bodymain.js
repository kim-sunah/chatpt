...
const Bodymain = () => {
    ...
    const turnedOn = true;
    const getBest = async () => {
        const res = await fetch('https://iamchatpt.com:444/payment/best');
        const weekBest_ = await res.json();
        if (weekBest_.length < 5) {
            const res2 = await fetch('https://iamchatpt.com:444/product/latest');
        ...
        if (turnedOn) {
            ...
            if (name) {
                const res = await fetch(`https://iamchatpt.com:444/payment/personalBest?key=${name}`);
    ...
    const getRating = async (id) => {
        const res = await fetch(`https://iamchatpt.com:444/comment/rating/${id}`);
