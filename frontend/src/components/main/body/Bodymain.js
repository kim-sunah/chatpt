...
const Bodymain = () => {
    ...
    const turnedOn = true;
    const getBest = async () => {
        const res = await fetch('https://localhost:4000/payment/best');
        const weekBest_ = await res.json();
        if (weekBest_.length < 5) {
            const res2 = await fetch('https://localhost:4000/product/latest');
        ...
        if (turnedOn) {
            ...
            if (name) {
                const res = await fetch(`https://localhost:4000/payment/personalBest?key=${name}`);
        ...
        try {
            const arr = await Promise.all(
                weekBest_.map(async (product) => {
                    const res = await fetch(`https://localhost:4000/comment/rating/${product.product_id}`);
    ...
    const getRating = async (id) => {
        const res = await fetch(`https://localhost:4000/comment/rating/${id}`);
