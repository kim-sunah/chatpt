...
const Allproduct = () => {
    ...
    useEffect(() => {
        fetch("iamchatpt.com:4430/payment/my", { method: "GET", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + sessionStorage.getItem("accessToken"), "refreshtoken": sessionStorage.getItem("refreshToken") } })
            .then(res => res.json())
            .then(resData => {
    ...
    useEffect(() => {
        Promise.all(productlist.map(async product => {
            const res = await fetch(`iamchatpt.com:4430/comment/rating/${product.product_id}`)
