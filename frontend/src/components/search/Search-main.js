...
const SearchMain = () => {
	...
	useEffect(() => {
		if (category !== "Category") {
			fetch("http://3.36.1.132:4000/product/categorysearch", {
				method: "POST",
				headers: {
	...
	useEffect(() => {
		fetch("http://3.36.1.132:4000/product/search", {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
			},
			...
			<main className="p-4">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 xl:grid-cols-5 gap-10 ">
					{productlist && productlist.map(product => (
						...
						<Link to={`http://3.36.1.132:3000/product/${product._source.id}`}><div key={product._source.id} className="rounded-lg border bg-card text-card-foreground shadow-sm w-full" data-v0-t="card">
