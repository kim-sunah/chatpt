...
const TrainerPage = () => {
    ...
    useEffect(() => {
        Promise.all(
            products[0].map(async (product) => {
                const res = await fetch(`iamchatpt.com/comment/rating/${product.id}`);
    ...
    const getUser = async () => {
        const res = await fetch('iamchatpt.com/users/Mypage', {
            method: 'GET',
    ...
    const getProduct = async () => {
        ...
        const res = await fetch(`iamchatpt.com/product/my?pageSize=100`, {
            method: 'GET',
            headers: {
                Authorization,
                refreshtoken,
    ...
    const getPayment = () => {
        ...
        const id = searchParams.get('id');
        return fetch(`iamchatpt.com/payment/${product_Id}`, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json', Authorization, refreshtoken },
        })
            .then((res) => {
    ...
    const getRevenue = async () => {
        ...
        try {
            const res = await fetch(`iamchatpt.com/payment/revenue/${product_Id}`, {
                method: 'GET',
                headers: { 'Content-Type': 'application/json', Authorization, refreshtoken },
            });
            if (res.status === 200) {
    ...
    const handleDeleteProduct = async (e, productId) => {
        ...
        if (window.confirm('정말로 이 강의를 삭제하시겠습니까?')) {
            try {
                const res = await fetch(`iamchatpt.com/product/${productId}`, {
                    method: 'DELETE',
    ...
    const getStudent = async () => {
        const res = await fetch(`iamchatpt.com/payment/${product_Id}`, {});
